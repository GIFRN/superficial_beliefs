...
def main() -> None:
    ...
    pair_consistency_rate = (
        float(tau_df.loc[tau_df["pairwise_ok"], "pairwise_consistent"].mean())
        if "pairwise_consistent" in tau_df and "pairwise_ok" in tau_df and tau_df["pairwise_ok"].any()
        else float("nan")
    )
    pair_cycle_rate = (
        float(tau_df.loc[tau_df["pairwise_ok"], "pairwise_has_cycle"].mean())
        if "pairwise_has_cycle" in tau_df and "pairwise_ok" in tau_df and tau_df["pairwise_ok"].any()
        else float("nan")
    )
    pair_mirror_complete_rate = (
        float(tau_df["pairwise_mirror_complete"].mean())
        if "pairwise_mirror_complete" in tau_df
        else float("nan")
    )
    pair_mirror_consistency_rate = (
        float(tau_df.loc[tau_df["pairwise_mirror_complete"], "pairwise_mirror_consistent"].mean())
        if "pairwise_mirror_complete" in tau_df and tau_df["pairwise_mirror_complete"].any()
        else float("nan")
    )
    pair_mirror_inconsistency_rate = (
        float(1.0 - pair_mirror_consistency_rate)
        if not np.isnan(pair_mirror_consistency_rate)
        else float("nan")
    )
    ...
    summary = {
        ...
        "pairwise_consistency_rate": pair_consistency_rate,
        "pairwise_cycle_rate": pair_cycle_rate,
        "pairwise_mirror_complete_rate": pair_mirror_complete_rate,
        "pairwise_mirror_consistency_rate": pair_mirror_consistency_rate,
        "pairwise_mirror_inconsistency_rate": pair_mirror_inconsistency_rate,
    ...
    report_lines = [
        ...
        f"- Pairwise consistency rate: {pair_consistency_rate:.3f}" if not np.isnan(pair_consistency_rate) else "- Pairwise consistency rate: n/a",
        f"- Pairwise cycle rate: {pair_cycle_rate:.3f}" if not np.isnan(pair_cycle_rate) else "- Pairwise cycle rate: n/a",
        f"- Pairwise mirror-complete rate: {pair_mirror_complete_rate:.3f}" if not np.isnan(pair_mirror_complete_rate) else "- Pairwise mirror-complete rate: n/a",
        f"- Pairwise mirror-consistency rate: {pair_mirror_consistency_rate:.3f}" if not np.isnan(pair_mirror_consistency_rate) else "- Pairwise mirror-consistency rate: n/a",
        f"- Pairwise mirror-inconsistency rate: {pair_mirror_inconsistency_rate:.3f}" if not np.isnan(pair_mirror_inconsistency_rate) else "- Pairwise mirror-inconsistency rate: n/a",
